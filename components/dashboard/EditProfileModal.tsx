...
export default function EditProfileModal({
  ...
  const realNamePreview = `${name.trim() || "Dani"}${lastInitialDraft.trim() ? ` ${lastInitialDraft.trim().charAt(0).toUpperCase()}.` : ""}`;
  ...
  return (
    <Modal open={open} onClose={close}>
      ...
      <div className="flex flex-col gap-2.5">
        {MODES.map((m) => {
          const selected = modeDraft === m.id;
          return (
            <div
              key={m.id}
              onClick={() => setModeDraft(m.id)}
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 text-xs leading-relaxed text-muted">
                    {m.body}{" "}
                    {m.id === "anon" ? <strong className="text-teal">{handlePreview}</strong> : null}
                  ...
                  {m.id === "nick" && selected ? (
                    <>
                      ...
                      />
                      <div className="mt-1 text-right text-[11px] text-muted-2">{nickDraft.length}/20</div>
                    </>
                  ) : null}
                    ...
                    <>
                      <div className="mt-2.5 flex items-center gap-2">
                        <input
                          ...
                          maxLength={1}
                          placeholder="L"
                          value={lastInitialDraft}
                          onChange={(e) => setLastInitialDraft(e.target.value)}
                          onClick={(e) => e.stopPropagation()}
                        />
